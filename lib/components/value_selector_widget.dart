        ...
        return Container(
          ...
          child: FutureBuilder<ApiCallResponse?>(
            future: allQuestionsCall(
              category: containerGamesRecord?.currentCategory ?? 0,
            ),
            builder: (context, snapshot) {
              // Customize what your widget looks like when it's loading.
              if (!snapshot.hasData) {
              ...
              return Builder(
                builder: (context) {
                  final questions = getJsonField(listViewAllQuestionsResponse?.jsonBody, r'''$''')?.toList() ?? [];
                      ...
                      return Stack(
                        children: [
                          if (containerGamesRecord?.answeredQuestions
                                  ?.contains(getJsonField(questionsItem, r'''$.id''')) ??
                              true)
                            Align(
                          ...
                          Align(
                            ...
                            child: InkWell(
                              onTap: () async {
                                final gamesUpdateData = {
                                  ...createGamesRecordData(
                                    questionMode: true,
                                    currentValue: getJsonField(questionsItem, r'''$.value'''),
                                    currentQuestion: getJsonField(questionsItem, r'''$.question''').toString(),
                                    currentAnswer: getJsonField(questionsItem, r'''$.answer''').toString(),
                                    selectMode: false,
                                  ),
                                  'answered_questions':
                                      FieldValue.arrayUnion([getJsonField(questionsItem, r'''$.id''')]),
                                };
                              ...
                              child: ListTile(
                                title: Text(
                                  getJsonField(questionsItem, r'''$.value''').toString(),
                                  style: FlutterFlowTheme.title3,
                                ),
                                tileColor: Color(0xFFF5F5F5),
                                dense: false,
                                contentPadding: EdgeInsetsDirectional.fromSTEB(15, 8, 15, 8),
                              ),
                            ),
                          ),
                        ],
